...
typedef struct {
    ...
    XDot_prod_uio_map maps[ MAX_UIO_MAPS ];
} XDot_prod_uio_info;

/***************** Variable Definitions **************************************/
static XDot_prod_uio_info uio_info;
...
static int uio_info_read_name(XDot_prod_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDot_prod_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDot_prod_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDot_prod_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDot_prod_Initialize(XDot_prod *InstancePtr, const char* InstanceName) {
	XDot_prod_uio_info *InfoPtr = &uio_info;
