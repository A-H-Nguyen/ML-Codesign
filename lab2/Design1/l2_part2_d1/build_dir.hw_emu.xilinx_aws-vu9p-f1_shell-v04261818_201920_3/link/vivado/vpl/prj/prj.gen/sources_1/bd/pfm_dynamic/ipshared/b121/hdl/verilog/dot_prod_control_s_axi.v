...
#(parameter
    ...
    output wire [63:0]                   vec_a,
    output wire [63:0]                   vec_b,
    output wire [63:0]                   results,
    output wire [31:0]                   vector_len,
...
localparam
    ...
    ADDR_IER               = 6'h08,
    ADDR_ISR               = 6'h0c,
    ADDR_VEC_A_DATA_0      = 6'h10,
    ADDR_VEC_A_DATA_1      = 6'h14,
    ADDR_VEC_A_CTRL        = 6'h18,
    ADDR_VEC_B_DATA_0      = 6'h1c,
    ADDR_VEC_B_DATA_1      = 6'h20,
    ADDR_VEC_B_CTRL        = 6'h24,
    ADDR_RESULTS_DATA_0    = 6'h28,
    ADDR_RESULTS_DATA_1    = 6'h2c,
    ADDR_RESULTS_CTRL      = 6'h30,
    ADDR_VECTOR_LEN_DATA_0 = 6'h34,
    ADDR_VECTOR_LEN_CTRL   = 6'h38,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_vec_a = 'b0;
    reg  [63:0]                   int_vec_b = 'b0;
    reg  [63:0]                   int_results = 'b0;
    reg  [31:0]                   int_vector_len = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_VEC_A_DATA_0: begin
                    rdata <= int_vec_a[31:0];
                end
                ADDR_VEC_A_DATA_1: begin
                    rdata <= int_vec_a[63:32];
                end
                ADDR_VEC_B_DATA_0: begin
                    rdata <= int_vec_b[31:0];
                end
                ADDR_VEC_B_DATA_1: begin
                    rdata <= int_vec_b[63:32];
                end
                ...
                ADDR_VECTOR_LEN_DATA_0: begin
                    rdata <= int_vector_len[31:0];
                end
            endcase
        end
    end
...
assign vec_a         = int_vec_a;
assign vec_b         = int_vec_b;
assign results       = int_results;
assign vector_len    = int_vector_len;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_A_DATA_0)
            int_vec_a[31:0] <= (WDATA[31:0] & wmask) | (int_vec_a[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_A_DATA_1)
            int_vec_a[63:32] <= (WDATA[31:0] & wmask) | (int_vec_a[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_B_DATA_0)
            int_vec_b[31:0] <= (WDATA[31:0] & wmask) | (int_vec_b[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_B_DATA_1)
            int_vec_b[63:32] <= (WDATA[31:0] & wmask) | (int_vec_b[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_vector_len[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_VECTOR_LEN_DATA_0)
            int_vector_len[31:0] <= (WDATA[31:0] & wmask) | (int_vector_len[31:0] & ~wmask);
    end
end
